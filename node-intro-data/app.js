...
// --- ROUTES ---
// there are no routes defined for / meaning if we go to ~/ it will be broken

// Dashboard
const dashboard = require('./routes/dashboard'); // Import the router as variable dashboard
app.use('/dashboard', dashboard); // Mount the routes root at /dashboard -> this connected in routes/dashboard.js

// member
const member = require('./routes/member'); // Import the router as variable dashboard
app.use('/member', member); // Mount the routes root at /member -- including /member/:id because its mentioned in the routes/member.js

// --- START SERVER ---
// Start the server and listen for connections
app.listen(port, () => {
  console.log(`💀 port ${port}`); //Backticks for dynamic strings and variables, single quotes for the basics strings only.
});